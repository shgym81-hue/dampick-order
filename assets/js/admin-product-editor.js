...
window.createDampickProductEditor = function (sb, onSaved) {
  ...
  const fields = {
    ...
    stock_quantity: document.getElementById("editProductStock")
  };
  ...
  form.addEventListener("submit", async event => {
    ...
    const price = String(fields.unit_price.value ?? "").trim();
    const stock = String(fields.stock_quantity.value ?? "").trim();
    const payload = {
    ...
    if (!payload.name || !payload.unit_name || !price || !Number.isSafeInteger(payload.unit_price) || payload.unit_price < 0 || !payload.pickup_date || stock === "" || !Number.isSafeInteger(payload.stock_quantity) || payload.stock_quantity < 0) {
      message.textContent = "상품 정보와 0 이상의 정수 재고 수량을 입력해주세요.";
      return;
    }
    busy = true;
    ...
    try {
      const { data, error } = await sb.from("products").update(payload)
        .eq("id", productId).eq("is_active", true).select("id,name,unit_price,unit_name,pickup_date,stock_quantity").single();
  ...
  return {
    open(product) {
      ...
      fields.stock_quantity.value = Number(product.stock_quantity ?? 0);
      message.textContent = "";
      dialog.showModal();
      fields.name.focus();
    }
